...
try {
    ...
    try {
        ...
        if (empty($citizens)) {
            $citizens = []; // Ensure we always return an array
        }

        sendJsonResponse(true, 'Citizens retrieved successfully', [
            'citizens' => $citizens,
            'count' => count($citizens)
        ]);

    } catch (PDOException $e) {
        error_log('Database error in citizens.php: ' . $e->getMessage());
        sendJsonResponse(false, 'Database error occurred. Please try again later.', null, 500);
    }

} catch (Exception $e) {
    error_log('Unexpected error in citizens.php: ' . $e->getMessage());
